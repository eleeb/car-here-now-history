...
**\*Note:** You may notice there is also directory titled `parker_icons` which contains 14 PNG files. These are custom icons I created to use in the 2 maps I created using Google My Maps (1 contains all the parking data separated by day, the other contains only entries from my current neighborhood by day). For privacy reasons, the URLs to these maps are not available publicly. Please reach out if you are interested in viewing them.
